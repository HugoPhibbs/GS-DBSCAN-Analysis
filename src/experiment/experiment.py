...
def run_gs_dbscan(datasetFilename, outFile, n, d, D, minPts, k, m, eps, alpha, distancesBatchSize, distanceMetric,
                  ...
                  useBatchDbscan=False, timeIt=False, useBatchABMatrices=False, useBatchNorm=False, dataset_dtype="f32",
                  miniBatchSize=10000, ABatchSize=10000, BBatchSize=128, normBatchSize=10000, sigmaEmbed=1):
    run_cmd = [
        EXEC_PATH,
        ...
        "--datasetDType", str(dataset_dtype),
        "--miniBatchSize", str(miniBatchSize),
        "--ABatchSize", str(ABatchSize),
        "--BBatchSize", str(BBatchSize),
        "--normBatchSize", str(normBatchSize),
        "--sigmaEmbed", str(sigmaEmbed)
    ]
...
def run_complete_sdbscan_pipeline(outFile="results.json", n=70000, d=784, D=1024, minPts=50, k=5, m=50, eps=0.11,
                                  ...
                                  clusterOnCpu=True,
                                  needToNormalize=True, print_cmd=True, datasetFilename=wm.ROW_MAJOR_FILENAME,
                                  labels_filename=None,
                                  verbose=False,
                                  useBatchDbscan=False,
                                  timeIt=False,
                                  useBatchABMatrices=False,
                                  useBatchNorm=False,
                                  dataset_dtype="f32",
                                  miniBatchSize=10000,
                                  ABatchSize=10000,
    ...
    run_gs_dbscan(datasetFilename,
                  ...
                  timeIt=timeIt,
                  useBatchABMatrices=useBatchABMatrices,
                  useBatchNorm=useBatchNorm,
                  dataset_dtype=dataset_dtype,
                  miniBatchSize=miniBatchSize,
                  ABatchSize=ABatchSize,
                  BBatchSize=BBatchSize,
                  normBatchSize=normBatchSize,
                  sigmaEmbed=sigmaEmbed)
